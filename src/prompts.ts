...
5. State the user-visible result before the goals: one concrete sentence naming what the human will see working when this plan is done. Take it from the original request, not from your implementation plan. If the user wants a working behavior, do not substitute a completed run, report, file or honest failure for that behavior. Every requested artifact and action must survive into this sentence. An agent-inferred constraint may not replace, defer or contradict it; ask the human if an inference would change the result. Do not present the review menu with a placeholder goal such as "work out the thing", "improve it" or "investigate".
...
Saving, redrafting, interview answers and Log updates never request acceptance. Do not print the entire plan again or call RequestPlanReview while a consequential protected choice still needs approval. Optional questions and implementation TODOs do not block acceptance; honor explicit shortcuts without inventing an interview. Only an intentional RequestPlanReview call (or the human's /goals review) displays the full draft and menu (Ready / Discuss / Edit / Cancel).
...
Right-size it:
- One goal per distinct judgeable outcome, not per experiment or report. Group related goals when it helps judge them together and readability. The count flows from the outcomes.
- Write each visible goal as a short, concrete requested deliverable or behavior that stands alone.
  ...
  For research, name the desired working behavior and what the user will see; put experiments, debugging and records under it. A negative run keeps the goal open unless the user explicitly asked only for that run.
