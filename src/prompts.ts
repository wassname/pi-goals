...
export const goalCheckInWake = "Goal check-in: only while supervising unfinished goals, start from the user-visible result, read the attached plan and inspect worker_view if available. Check for drift and stuck/stopped/blocked work; ensure follow-up. A stopped worker normally needs a direct steer, retry or permitted in-flight plan edit. Use formal review only when you choose to allow it to stop. When something materially changed, tell the user the goal, what the result means and the next action in a few sentences; ask only when needed. If nothing changed, say so in one line and slow the cadence for a reliable followed job. For a substantive update, sample one random line with `shuf -n 1 ~/.pi/agent/skills/ml-debug/fortune.txt`; a kaomoji is welcome. Skip it for an unchanged wait. Do not replay completed work. Never create a timer from this wake.";
